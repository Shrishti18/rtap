...
"""
import re  # noqa: F401  (used by read_wout_centres; missing in the shipped file)

import numpy as np

KB_MEV = 0.086173  # meV/K
C2D = 0.068        # D_s ~ C2D * U * M, calibrated in-class (2D). See calib note.
...
def spec(dsc, Tc_K=300.0, dim=2):
    """Required U and metric threshold for target Tc. Energies in eV."""
    Tc = Tc_K * KB_MEV / 1000.0
    lam = dsc['lam']
    U_req = 4 * Tc / lam                       # from Tc_MF = U*lam/4
    if dim == 2:
        M_req = 2 * lam / (np.pi * C2D)        # from (pi/8)*C2D*U*M >= Tc
        cal = True
    else:
        M_req = 2 * lam / (np.pi * C2D) / 5.0  # 3D ~5x looser; coefficient UNCALIBRATED
        cal = False
    return dict(Tc_K=Tc_K, U_req=U_req, iso_req=2 * U_req, M_req=M_req,
                pass_iso=dsc['d_iso'] >= 2 * U_req,
                pass_M=dsc['M'] >= M_req,
                pass_unif=dsc['unif'] < 0.05,
                calibrated=cal)


def auto_bands(Hk, wmax=0.5, iso_min=0.2):
    """Candidate isolated narrow bands."""
...
# ---------------- flat-band-pair criterion (2-orbital manifold) ----------------

def pair_criterion(R, H, deg, bands, nk=40, dim=3, U=0.207):
    """
    For a 2-band manifold: decompose H(k) = eps0(k)*1 + d(k).sigma.
    Derived requirements (U in eV, defaults to the 300 K value):
        min 2|d(k)| >= 2U        isolation of the two bands
        spread(eps0) <= 0.5*U    flatness (pairing dies by W ~ 2U)
        max|d|/min|d| - 1 <= 0.2 |d| near-constant
    Equivalently: inter-orbital (sigma) hopping must dominate the
    orbital-averaged (identity) hopping by a factor of roughly 4-8.
    """
    Hk, dks = hk_grid(R, H, deg, nk, dim)
    sub = np.ix_(*([np.arange(s) for s in Hk.shape[:-2]] + [bands, bands]))
    h = Hk[sub] if len(bands) == 2 else Hk[..., bands, :][..., :, bands]
    e0 = np.real(0.5 * (h[..., 0, 0] + h[..., 1, 1]))
    dz = np.real(0.5 * (h[..., 0, 0] - h[..., 1, 1]))
    dx = np.real(h[..., 0, 1])
    dy = -np.imag(h[..., 0, 1])
    dmag = np.sqrt(dx**2 + dy**2 + dz**2)
    dzh = np.mean(dz / np.maximum(dmag, 1e-12))
    W = float(e0.max() - e0.min())
    return dict(gap_min=float(2 * dmag.min()), W_eps0=W,
                d_var=float(dmag.max() / max(dmag.min(), 1e-12) - 1),
                dzhat=float(dzh),
                ratio=float(dmag.mean() / max(np.abs(e0).mean(), 1e-12)),
                pass_iso=2 * dmag.min() >= 2 * U,
                pass_flat=W <= 0.5 * U,
                pass_const=(dmag.max() / max(dmag.min(), 1e-12) - 1) <= 0.2,
                pass_unif=abs(dzh) < 0.05)


# ---------------- minimal quantum metric (Huhtinen et al.) ----------------

def _trg_shifted(P, dks, kgrids, shifts):
    """P_ab -> exp(i k.(d_a - d_b)) P_ab, then tr g. No re-diagonalisation needed."""
    ph = sum(kgrids[i][..., None] * shifts[:, i] for i in range(len(dks)))  # (...,norb)
    V = np.exp(1j * ph)
...
def minimal_metric(Hk, dks, band, nk, dim, centres=None, restarts=6, seed=0):
    """
    Minimise <tr g> over orbital position assignments -> M_min, a genuine
    band invariant. The naive value (shifts=0) is only an UPPER BOUND.
    If `centres` (fractional Wannier centres, shape (norb,dim)) are supplied
    they are used as a starting point and also evaluated directly.
    """
    from scipy.optimize import minimize as _min
    P, _ = projector(Hk, [band])
    norb = P.shape[-1]
    ks = [2 * np.pi * (np.arange(nk) + 0.5) / nk for _ in range(dim)]
    kgrids = np.meshgrid(*ks, indexing='ij')
    naive = float(_trg_shifted(P, dks, kgrids, np.zeros((norb, dim))))

    def obj(x):
        s = np.zeros((norb, dim))
        s[1:] = x.reshape(norb - 1, dim)
        return float(_trg_shifted(P, dks, kgrids, s))
...
def read_wout_centres(path):
    """Final Wannier centres (Cartesian, Angstrom) from a wannier90.wout."""
